...
def get_all_metrics(preds, labels, task, los_info):
    ...
    if task == "outcome":
        ...
        return get_binary_metrics(preds, labels[:, 0]) | es_score(
            labels[:, 0], y_true_los, preds, threshold
        )
    elif task == "los":
        ...
        if not isinstance(threshold, list):
            ...
            y_true_los = reverse_los(labels[:, 1], los_info)
            return (
                get_binary_metrics(preds[:, 0], labels[:, 0])
                | get_regression_metrics(y_pred_los, y_true_los)
                | osmae_score(
                    labels[:, 0],
                    y_true_los,
                    preds[:, 0],
                ...
                | es_score(labels[:, 0], y_true_los, preds[:, 0], threshold)
            )
        else:
            y_pred_los = reverse_los(preds[:, 1], los_info)
            y_true_los = reverse_los(labels[:, 1], los_info)
            ...
            for thr in threshold:
                ...
                es_list.append(
                    es_score(labels[:, 0], y_true_los, preds[:, 0], thr)["es"]
                )
